...
contract OpynPerpVault is ERC20, ReentrancyGuard, Ownable {
  ...
  function totalETHControlled() external view returns (uint256) {
    ...
    return
      totalStakedaoAsset()
        .mul(sdecrv.getPricePerFullShare())
  ...
  function depositETH(uint256 minEcrv) external payable nonReentrant {
    ...
    uint256 sdecrvDeposited = totalWithDepositedAmount.sub(
      totalSdecrvBalanceBeforeDeposit
    );
    uint256 share = _getSharesByDepositAmount(
      sdecrvDeposited,
      totalSdecrvBalanceBeforeDeposit
    );
  ...
  function withdrawETH(uint256 _share, uint256 minEth) external nonReentrant {
    ...
    uint256 ethReceived = curvePool.remove_liquidity_one_coin(
      ecrvBalance,
      0,
      minEth
    );
    ...
    nonReentrant
  {
    ...
    for (uint256 i = 0; i < _allocationPercentages.length; i = i + 1) {
      ...
      require(sumPercentage <= 10000, "O14");

      uint256 newAmount = totalAsset.mul(_allocationPercentages[i]).div(10000);

      if (newAmount > 0)
        IERC20(sdecrvAddress).safeTransfer(actions[i], newAmount);
      IAction(actions[i]).rolloverPosition();
    }

    require(sumPercentage == 10000, "O15");
   ...
   */
  function setWithdrawalFeeRecipient(address _newWithdrawalFeeRecipient)
    external
    onlyOwner
  {
   ...
   */
  function setWithdrawalFeePercentage(uint256 _newWithdrawalFeePercentage)
    external
    onlyOwner
  {
   ...
   */
  function _getSharesByDepositAmount(uint256 _amount, uint256 _totalAssetAmount)
    internal
    view
    returns (uint256)
  {
   ...
   */
  function _getWithdrawFee(uint256 _withdrawAmount)
    internal
    view
    returns (uint256)
  {
    return _withdrawAmount.mul(withdrawalFeePercentage).div(10000);
  }
